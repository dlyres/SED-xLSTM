...
def main(args):
    ...
    print(f"整体数据集大小为{data_size}")

    # 检查是否有可用的 GPU
    if torch.cuda.is_available():
        gpu_name = torch.cuda.get_device_name(0)
        print(f"训练的GPU为：{gpu_name}")
    else:
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(range(data_size))):
        ...
        print(f'训练集大小为{len(train_subset)}')
        print(f'验证集大小为{len(val_subset)}')
        ...
        for epoch in range(args.epochs):
            ...
            if args.model_name == 'lsvt_model' or args.model_name == 'grvt_model' or args.model_name == 'SSGFormer_model':
    ...
    print(f'每折的准确率: {acc_list}, 平均准确率: {avg_acc:.4f}')
    print(f'每折的auc: {auc_list}, 平均auc: {avg_auc:.4f}')
    print(f'每折的itr: {itr_list}, 平均itr: {avg_itr:.2f}')
