...
def main(args):
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(range(data_size))):
        ...
        train_step = 0
        test_step = 0
        final_class_acc = np.zeros(args.num_classes)  # final each class accuracy
        total_acc_list = []  # total accuracy trend
        ...
        for epoch in range(args.epochs):
            ...
            if args.model_name == 'VIT' or args.model_name == 'VGT' or args.model_name == 'SSGFormer_model':
        ...
        print("{} model training time:{:.2f} min".format(args.model_name, end_time / 60))
    ...
    # print kfolds average acc, auc, itr
    avg_acc = sum(acc_list) / args.k_folds
    avg_auc = sum(auc_list) / args.k_folds
    avg_itr = sum(itr_list) / args.k_folds
    print(f'each fold acc: {acc_list}, average acc: {avg_acc:.4f}')
    print(f'each fold auc: {auc_list}, average auc: {avg_auc:.4f}')
    print(f'each fold itr: {itr_list}, average itr: {avg_itr:.2f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--factor', type=float, default=0.5)  # decay rate
    ...
    # accuracy training trend diagram, roc
    ...
    if args.num_models == 1:
        ...
        model_list = ['spaFormer', 'grvt', 'plfa']
        model_acc_cross = []
        model_auc_cross = []
        for model_name in model_list:
            ...
            print(f"{args.model_name} model training finished")
        if args.accuracy:
