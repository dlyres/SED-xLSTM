...
def show(total_acc_list, final_class_acc, args, model_list=None, num_fold=None):
    ...
    if args.num_models == 1:
        ...
        a[0].set_title(f"dataset: {args.dataset_name}, {args.model_name} model accuracy training trend", y=1.05)
        a[0].plot(x1, y1)
        ...
        a[1].set_title(f"dataset: {args.dataset_name}, {args.model_name} model each class accuracy", y=1.05)
    ...
    # 多模型非交叉验证
        ...
        width = 0
        color_list = ['red', 'blue', 'orange', 'green', 'cyan']
