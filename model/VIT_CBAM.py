...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes=2, ratio=4):
...
class SpatialAttention(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=(7, 7), stride=(1, 1), padding='same')
        self.sigmoid = nn.Sigmoid()
...
class VIT_CBAM(nn.Module):
    def __init__(self, img_size, patch_size, in_c, num_classes,
                 embed_dim, depth, num_heads, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.2,
                 attn_drop_ratio=0.2, drop_path_ratio=0.2, embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None):
        ...
        self.patch_embed = embed_layer(img_size=self.img_size, patch_size=self.patch_size, in_c=self.in_c,
...
def make_model(args):
    if args.dataset_name == 'BETA' or args.dataset_name == 'Benchmark':
        model = VIT_CBAM(img_size=(30, 256),
