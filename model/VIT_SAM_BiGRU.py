...
class Vit_attn(nn.Module):
    def __init__(self, img_size, patch_size, in_c, embed_dim, depth, num_heads, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, drop_ratio=0.2, attn_drop_ratio=0.2, drop_path_ratio=0.5, norm_layer=None,
                 act_layer=None):
...
class VSG(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.gru((x + x_spa).transpose(0, 1))
        return x


def make_model(args):
    if args.dataset_name == 'BETA' or args.dataset_name == 'Benchmark':
        model = VSG(num_classes=args.num_classes,
                    in_c=args.in_c,
                    img_size=(30, 256),
